...
class Crud_Pro_Adminhtml_ProController extends Mage_Adminhtml_Controller_Action
{
	...
	public function editAction()
	{
		...
		if ($proModel->getId() || $proId == 0) 
		{
			...
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Student Information'), Mage::helper('adminhtml')->__('Student Information'));
 			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Data News'), Mage::helper('adminhtml')->__('Data News'));
			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
			$this->_addContent($this->getLayout()->createBlock('pro/adminhtml_pro_edit'))->_addLeft($this->getLayout()->createBlock('pro/adminhtml_pro_edit_tabs'));
			$this->renderLayout();
 		} 
 		else 
 		{
	...
	*/
	...
	{
		$this->loadLayout();
		$this->getResponse()->setBody($this->getLayout()->createBlock('pro/adminhtml_pro_grid')->toHtml());
	}
}